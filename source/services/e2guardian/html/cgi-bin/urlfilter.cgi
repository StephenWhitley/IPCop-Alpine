...
my $dbdir      = "/var/ipcop/proxy/urlfilter";
    ...
    if ($filtersettings{'ACTION'} eq $Lang::tr{'upload'}) {
        ...
        if ($exitcode > 0) {
            ...
            if (!(-d "/var/ipcop/proxy/update/blacklists")) {
                ...
                `/usr/bin/blacklistupdate.pl --perm < /dev/null > /dev/null &`;
                system(
...
if (($besettings{'ACTION'} eq $Lang::tr{'install blacklist'}) && ($besettings{'MODE'} = 'BLACKLIST_EDITOR')) {
    ...
    if ($besettings{'BE_NAME'} eq '') {
        ...
        # ufdbGuard compilation
        my $ugargs = "-n -D -W -t " . $besettings{'BE_NAME'};
        if ($besettings{'BE_DOMAINS'}) {
            $ugargs .= " -d $dbdir/$besettings{'BE_NAME'}/domains";
        }
        if ($besettings{'BE_URLS'}) {
            $ugargs .= " -u $dbdir/$besettings{'BE_NAME'}/urls";
        }
        # ufdbGenTable doesn't strictly need expressions file argument if expressions are just lines in domains/urls?
        # Actually ufdbGuard handles expressions file differently? 
        # Checking logic: "domainlist domains", "urllist urls", "expressionlist expressions"
        # ufdbGenTable takes -d, -u. Does it take -e?
        # Typically expressions are compiled into the database or separate.
        # ufdbGenTable usage: -e <expressions file>
        if ($besettings{'BE_EXPRESSIONS'}) {
            $ugargs .= " -e $dbdir/$besettings{'BE_NAME'}/expressions";
        }

        if ($besettings{'BE_DOMAINS'} || $besettings{'BE_URLS'} || $besettings{'BE_EXPRESSIONS'}) {
             system("/usr/bin/ufdbGenTable $ugargs");
        }
...
if ($filtersettings{'ACTION'} eq $Lang::tr{'instant update'}) {
    if ($filtersettings{'UPDATE_SOURCE'} eq 'custom' && $filtersettings{'CUSTOM_UPDATE_URL'} eq '') {
        ...
        `/usr/bin/blacklistupdate.pl --force < /dev/null > /dev/null &`;
        sleep(1);
    }
}
...
if ((!$tcsettings{'TCMODE'}) && (!$uqsettings{'UQMODE'}) && (!$besettings{'BEMODE'})) {
    ...
    print "<div style='background-color: #d1e7dd; color: #0f5132; padding: 10px; border: 1px solid #badbcc; border-radius: 4px; margin-bottom: 10px; text-align: center;'><b>Running in e2guardian Parent Proxy Mode</b></div>";
    my $sactive = "<td align='center' class='ipcop_stopped'>$Lang::tr{'stopped'}</td>";
    if ($filtersettings{'ENABLED'} eq 'on') {
        $sactive = &General::isrunning('e2guardian', 'nosize');
    }
...
<tr>
    <td colspan='4'><b>$Lang::tr{'content screening'}</b></td>
</tr>
<tr>
    <td class='base'>$Lang::tr{'weighted phrase mode'}:</td>
    <td><input type='checkbox' name='ENABLE_PHRASES' $checked{'ENABLE_PHRASES'}{'on'} /></td>
    <td class='base'>$Lang::tr{'naughtiness limit'}:</td>
    <td><input type='text' name='PHRASE_LIMIT' value='$filtersettings{'PHRASE_LIMIT'}' size='5' /> (Default: 50)</td>
</tr>
<!--
<tr>
...
sub setpermissions
{
    ...
    system("chown -R squid:lighttpd $bldir");

    foreach $category (<$bldir/*>) {
        if (-d $category) {
            system("chmod 775 $category &> /dev/null");
            foreach $blacklist (<$category/*>) {
                if (-f $blacklist) { system("chmod 664 $blacklist &> /dev/null"); }
                if (-d $blacklist) { system("chmod 775 $blacklist &> /dev/null"); }
            }
            system("chmod 664 $category/*.db &> /dev/null");
...
sub writeconfigfile
{
    ...
    # --- Custom Blocked Lists ---
    open(FILE, ">$dbdir/bannedsitelist");
    print FILE "# Bannedsitelist generated by IPCop\n";
    ...
    my @categories = &readblockcategories; # Ensure we have category list
    foreach $category (@categories) {
        $blacklist = $category;
        # Check if category is enabled in settings (filtergroups[i] mapping logic needed if strictly following old UI)
        # For simplicity, we assume categories are handled by the update logic into the lists directory
        # IPCop's UI logic maps categories to filtergroups array.
        # We need to check if the category is enabled in $filtersettings
        
        # Note: IPCop's category/group logic is complex. 
        # For e2guardian, we'll simply include the list file if the category is 'on'
        # But wait, categories are dynamically loaded.
        
        # Simplified: We assume standard blacklist structure in /etc/e2guardian/lists/blacklists/
        # and we include them here.
        
        # Checking if category is enabled:
        # The UI checkboxes map to variable names equal to category names usually?
        # Let's check how IPCop names them. In readblockcategories it populates @filtergroups.
        # Let's iterate all filter settings keys.
        
        if (defined($filtersettings{$category}) && $filtersettings{$category} eq 'on') {
             if (-f "$dbdir/blacklists/$category/domains") {
                 print FILE ".Include<$dbdir/blacklists/$category/domains>\n";
             }
        }
    }
    close(FILE);
    ...
    # --- Custom Blocked URLs ---
    open(FILE, ">$dbdir/bannedurllist");
    print FILE "# Bannedurllist generated by IPCop\n";
    if(defined($filtersettings{'CUSTOM_BLACK_URLS'})) {
    ...
    foreach $category (@categories) {
        if (defined($filtersettings{$category}) && $filtersettings{$category} eq 'on') {
             if (-f "$dbdir/blacklists/$category/urls") {
                 print FILE ".Include<$dbdir/blacklists/$category/urls>\n";
             }
        }
    }
    close(FILE);

    # --- Custom Exceptions (Whitelist) ---
    open(FILE, ">$dbdir/exceptionsitelist");
    print FILE "# Exceptionsitelist generated by IPCop\n";
    if(defined($filtersettings{'CUSTOM_WHITE_DOMAINS'})) {
    ...
    open(FILE, ">$dbdir/exceptionurllist");
    print FILE "# Exceptionurllist generated by IPCop\n";
    if(defined($filtersettings{'CUSTOM_WHITE_URLS'})) {
    ...
    # --- Custom Explicit Expression List ---
    open(FILE, ">$dbdir/bannedregexpurllist");
    print FILE "# Bannedregexpurllist generated by IPCop\n";
    if(defined($filtersettings{'CUSTOM_EXPRESSIONS'})) {
    ...
    open(FILE, ">$dbdir/bannedextensionlist");
    print FILE "# Bannedextensionlist generated by IPCop\n";
    if ($filtersettings{'BLOCK_EXECUTABLES'} eq 'on') { 
        print FILE "# Block Executables\n.exe\n.com\n.bat\n.dll\n.scr\n.msi\n"; 
    }
    if ($filtersettings{'BLOCK_AUDIO-VIDEO'} eq 'on') { 
        print FILE "# Block Audio/Video\n.mp3\n.mpeg\n.mpg\n.avi\n.mov\n.wmv\n.asf\n.mp4\n"; 
    }
    if ($filtersettings{'BLOCK_ARCHIVES'} eq 'on') { 
        print FILE "# Block Archives\n.zip\n.tar\n.gz\n.tgz\n.rar\n.iso\n"; 
    }
    close(FILE);

    # --- Weighted Phrase Lists ---
    open(FILE, ">$dbdir/weightedphraselist");
    print FILE "# Weightedphraselist generated by IPCop\n";
    if (($filtersettings{'ENABLE_PHRASES'} eq 'on') && (-f "/etc/e2guardian/lists/weightedphraselist")) {
        print FILE ".Include</etc/e2guardian/lists/weightedphraselist>\n";
    }
    close(FILE);

    open(FILE, ">$dbdir/bannedphraselist");
    print FILE "# Bannedphraselist generated by IPCop\n";
    if (($filtersettings{'ENABLE_PHRASES'} eq 'on') && (-f "/etc/e2guardian/lists/bannedphraselist")) {
        print FILE ".Include</etc/e2guardian/lists/bannedphraselist>\n";
    }
    close(FILE);

    open(FILE, ">$dbdir/exceptionphraselist");
    print FILE "# Exceptionphraselist generated by IPCop\n";
    if (($filtersettings{'ENABLE_PHRASES'} eq 'on') && (-f "/etc/e2guardian/lists/exceptionphraselist")) {
        print FILE ".Include</etc/e2guardian/lists/exceptionphraselist>\n";
    }
    close(FILE);

    # Generate the main e2guardian configuration
    &write_e2guardian_conf();

}

sub write_e2guardian_conf
{
    my $phraselimit = $filtersettings{'PHRASE_LIMIT'};
    if ($phraselimit eq '') { $phraselimit = 50; }

    open(CONF, ">/var/ipcop/proxy/e2guardian.conf");
    print CONF <<END
# IPCop e2guardian configuration
# Generated by urlfilter.cgi
...
# Content Screening Limits
weightedphraselimit = $phraselimit

# List Paths
bannedsitelist = '/var/ipcop/proxy/urlfilter/bannedsitelist'
bannedurllist = '/var/ipcop/proxy/urlfilter/bannedurllist'
exceptionsitelist = '/var/ipcop/proxy/urlfilter/exceptionsitelist'
exceptionurllist = '/var/ipcop/proxy/urlfilter/exceptionurllist'
bannedregexpurllist = '/var/ipcop/proxy/urlfilter/bannedregexpurllist'
bannedextensionlist = '/var/ipcop/proxy/urlfilter/bannedextensionlist'

weightedphraselist = '/var/ipcop/proxy/urlfilter/weightedphraselist'
bannedphraselist = '/var/ipcop/proxy/urlfilter/bannedphraselist'
exceptionphraselist = '/var/ipcop/proxy/urlfilter/exceptionphraselist'
...
sub restartexit
{
    ...
    system('/usr/bin/restartsquid --waitpid >/dev/null &') if ($restart);
