...
/*
    Accept backups from older versions, unless the backup is very old with too
   many differences (< v1.9.9) Accept backups from newer versions, where only
   revision (last number) is different, v2.0.x will accept v2.0.y
*/
int testbackupversion(char *path) {
  ...
  if (backupversion == 0) {
    FILE *out = flog ? flog : stderr;
    fprintf(out, "Backup version missing\n");
    return FAILURE;
  }
  ipcopversion = getipcopversion();
  if (ipcopversion == 0) {
    FILE *out = flog ? flog : stderr;
    fprintf(out, "IPCop version missing\n");
    return FAILURE;
  }

  if (backupversion < 0x010909) {
    FILE *out = flog ? flog : stderr;
    fprintf(out, "Backup 0x%08X too old\n", backupversion);
    return FAILURE;
  }
  ...
  FILE *out = flog ? flog : stderr;
  fprintf(out, "Backup: 0x%08X  >>  IPCop: 0x%08X\n", backupversion,
          ipcopversion);
  return FAILURE;
}
