...
void config() {
  ...
  snprintf(command, STRING_SIZE, "/usr/bin/makesquidconf.pl");
  safe_system(command);
  ...
  while (fgets(s, STRING_SIZE_LARGE, file) != NULL) {
    ...
    if (setdirectipsec_green) {
      if (snprintf(buffer, STRING_SIZE - 1,
                   "/usr/sbin/iptables -t nat -A SQUID -i %s -p tcp -d "
                   "%s/%s --dport 80 -j RETURN",
                   ipcop_ethernet.device[GREEN][1], ipsec_netaddress,
                   ipsec_netmask) >= STRING_SIZE) {
        fprintf(stderr, "Command too long\n");
        fclose(file);
        exit(1);
      }
      verbose_printf(
          1, "Bypass proxy redirect for GREEN to remote IPsec network %s/%s\n",
    ...
    if (setdirectipsec_blue) {
      if (snprintf(buffer, STRING_SIZE - 1,
                   "/usr/sbin/iptables -t nat -A SQUID -i %s -p tcp -d "
                   "%s/%s --dport 80 -j RETURN",
                   ipcop_ethernet.device[BLUE][1], ipsec_netaddress,
                   ipsec_netmask) >= STRING_SIZE) {
        fprintf(stderr, "Command too long\n");
        fclose(file);
        exit(1);
      ...
      verbose_printf(
          1, "Bypass proxy redirect for BLUE to remote IPsec network %s/%s\n",
          ipsec_netaddress, ipsec_netmask);
      safe_system(buffer);
    }
  }
...
int main(int argc, char **argv) {
  ...
  static struct option long_options[] = {
      {"flush", no_argument, 0, 'f'},   {"repair", no_argument, 0, 'r'},
      {"test", no_argument, 0, 't'},    {"config", no_argument, 0, 'c'},
      {"waitpid", no_argument, 0, 'w'}, {"verbose", no_argument, 0, 'v'},
  ...
  if (access("/var/run/squid.pid", F_OK) != -1) {
    ...
    safe_system("/usr/sbin/iptables -t nat -F SQUID");
  ...
  if (flag_repair) {
    ...
    if ((pw = getpwnam("squid"))) {
      ...
      if (stat("/var/log/cache/swap.state", &st) == 0) {
        unpriv_system("/bin/rm -f /var/log/cache/swap.state", pw->pw_uid,
                      pw->pw_gid);
      }
      saferestart = 1;
    } else {
  ...
  if (enabled_green || enabled_blue || enabled_ovpn) {
    ...
    if (safe_system("/usr/sbin/squid -s -z -N 2>/dev/null")) {
      verbose_printf(1, "Error creating swap directories ... \n");
    }
    sleep(1);

    verbose_printf(1, "Restarting e2guardian ... \n");
    safe_system("/sbin/rc-service e2guardian restart >/dev/null 2>/dev/null");
  ...
  if (strcmp(ipcop_ethernet.red_type[1], "STATIC") == 0) {
    snprintf(destination, STRING_SIZE, "%s/%s", ipcop_ethernet.address[RED][1],
             ipcop_ethernet.netmask[RED][1]);
  } else {
    if (ipcop_ethernet.red_address[1][0] &&
        VALID_IP(ipcop_ethernet.red_address[1])) {
  ...
  if (transparent_green && enabled_green) {
    ...
    if (snprintf(buffer, STRING_SIZE - 1,
                 "/usr/sbin/iptables -t nat -A SQUID -i %s -p tcp -d %s "
                 "--dport 80 -j RETURN",
    ...
    if (snprintf(buffer, STRING_SIZE - 1,
                 "/usr/sbin/iptables -t nat -A SQUID -i %s -p tcp --dport "
                 "80 -j REDIRECT --to-port %d",
                 ipcop_ethernet.device[GREEN][1],
                 PORT_PROXY_INTERCEPT) >= STRING_SIZE) {
      fprintf(stderr, "Command too long\n");
      exit(1);
    }
    safe_system(buffer);
  ...
  if (transparent_ovpn && enabled_ovpn) {
    ...
    verbose_printf(1,
                   "Setting transparent iptables rule for OpenVPN RW ... \n");
    if (snprintf(buffer, STRING_SIZE - 1,
                 "/usr/sbin/iptables -t nat -A SQUID -i %s -p tcp -d %s "
                 "--dport 80 -j RETURN",
