...
$proxysettings{'ERR_DESIGN'} = 'squid';
...
ERROR:
        ...
        if ($proxysettings{'VALID'} eq 'yes')
        {
            ...
            system('/usr/bin/restartsquid --config >/dev/null 2>&1');

            system('/usr/bin/restartsquid --waitpid >/dev/null &');
    ...
    if ($proxysettings{'ACTION'} eq $Lang::tr{'clear cache'}) {
        system('/usr/bin/restartsquid --flush --waitpid >/dev/null 2>&1');
    }
...
sub adduser
{
    ...
    if ($str_pass eq 'lEaVeAlOnE')
    {
        ...
        &deluser($str_user);
        system("/usr/sbin/htpasswd -b $userdb $str_user $str_pass >/dev/null 2>&1");
    }
