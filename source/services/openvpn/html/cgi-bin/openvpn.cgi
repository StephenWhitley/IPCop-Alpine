...
my @serverstatus = `/usr/bin/restartopenvpn --status`;
...
sub emptyserverlog{
    if (open(FILE, ">/var/log/openvpnserver.log")) {
        ...
        print FILE "";
        close FILE;
        chmod 0660, "/var/log/openvpnserver.log";
    }
}
...
if ($cgiparams{'ACTION'} eq $Lang::tr{'start openvpn server'} ||
    ...
    if ($cgiparams{'ACTION'} eq $Lang::tr{'start openvpn server'}) {
        &emptyserverlog();
        system('/usr/bin/restartopenvpn --start >/dev/null 2>&1');
    }

    #stop openvpn server
    if ($cgiparams{'ACTION'} eq $Lang::tr{'stop openvpn server'}) {
        system('/usr/bin/restartopenvpn --stop >/dev/null 2>&1');
        &emptyserverlog();
    }

    #restart openvpn server
    if ($cgiparams{'ACTION'} eq $Lang::tr{'restart openvpn server'}) {
        system('/usr/bin/restartopenvpn --restart >/dev/null 2>&1');
        &emptyserverlog();
    }
}
...
###
    ...
    if ($confighash{$cgiparams{'KEY'}}) {
        system("/usr/bin/openssl ca -revoke /var/ipcop/openvpn/certs/$confighash{$cgiparams{'KEY'}}[1]cert.pem >/dev/null 2>&1");
        ...
        system("/usr/bin/openssl ca -gencrl -out /var/ipcop/crls/cacrl.pem >/dev/null 2>&1");
        &General::writehasharray('/var/ipcop/openvpn/config', \%confighash);
    }
    else {
        $errormessage = $Lang::tr{'invalid key'};
...
if ( $srunning eq "yes" ) {
    ...
    if (( -e '/var/ipcop/ca/cacert.pem' &&
          -e '/var/ipcop/private/dh2048.pem' &&
