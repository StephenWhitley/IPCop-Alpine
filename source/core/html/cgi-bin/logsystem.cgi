...
'(red|connectioncheck|kernel: usb.*|pppd\[\d+\]|chat\[\d+\]|pppoe\[\d+\]|pptp\[\d+\]|pppoa\[\d+\]|pppoa3\[\d+\]|pppoeci\[\d+\]|ipppd|ipppd\[\d+\]|kernel: ippp\d|kernel: isdn.*|ibod\[\d+\]|kernel: eth.*|dhcpcd\[\d*\]|modem_run\[\d+\])',
    ...
    'openvpn'        => '(vpn|openvpn|openvpn\[\d+\]|OVPN_.*|openvpnserver\[\d+\])',
    'squid'          => '(squid\[\d+\]|msnt_auth\[\d+\])',
    'squidguard'     => '(squidGuard\[\d+\]|e2guardian\[\d+\])',
...
while ($loop) {
    ...
    if (open(FILE, ($filestr =~ /.gz$/ ? "gzip -dc $filestr |" : $filestr))) {
        ...
        READ: while (<FILE>) {
            my $line = $_;
            # Alpine/Busybox syslog includes 'facility.priority' (e.g. daemon.info) between hostname and tag
            # We add (?:[\w\.]+\s+)? to optionally match and ignore this field
            if ($line =~ /^${monthstr} ${daystr} ..:..:.. [\w\-]+ (?:[\w\.]+\s+)?${section}: (.*)/) {
