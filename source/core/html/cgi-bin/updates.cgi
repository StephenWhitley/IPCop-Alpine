...
if ($uploadsettings{'ACTION'} eq $Lang::tr{'upload'}) {
    ...
    if ($uploadsettings{'FH'} eq '') {
       ...
       &General::log("installpackage", "Upload error: $errormessage");
        ...
        my $ret = system("/usr/bin/installpackage --test=/var/patches/$uploadsettings{'FH'} >/dev/null") >> 8;
    ...
    system("/usr/bin/installpackage --install=/var/patches/${filename} >/dev/null &");
}
