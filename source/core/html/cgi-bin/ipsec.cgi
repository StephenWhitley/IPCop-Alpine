...
if ($cgiparams{'ACTION'} eq $Lang::tr{'save'} && $cgiparams{'TYPE'} eq '' && $cgiparams{'KEY'} eq '') {
    ...
    if($errormessage) {
        ...
        if (&VPN::ipsecenabled(\%vpnsettings)) {
            ...
            system('/usr/bin/ipsecctrl --start >/dev/null 2>&1');
        }
        else {
            &General::log("ipsec", "Stop ipsecctrl");
            system('/usr/bin/ipsecctrl --stop >/dev/null 2>&1');
        }
        sleep $sleepDelay;
    }
}
    ...
    if ($confighash{$cgiparams{'KEY'}}) {
        if ($confighash{$cgiparams{'KEY'}}[0] eq 'off') {
            ...
            if (&VPN::ipsecenabled(\%vpnsettings)) {
                ...
                system("/usr/bin/ipsecctrl --start=$cgiparams{'KEY'} >/dev/null 2>&1")
            }
        }
        else {
            if (&VPN::ipsecenabled(\%vpnsettings)) {
                &General::log("ipsec", "Deactivate connection #$cgiparams{'KEY'}");
                system("/usr/bin/ipsecctrl --stop=$cgiparams{'KEY'} >/dev/null 2>&1")
            }
...
###
    ...
    if ($confighash{$cgiparams{'KEY'}}) {
        if (&VPN::ipsecenabled(\%vpnsettings)) {
            ...
            system("/usr/bin/ipsecctrl --start=$cgiparams{'KEY'}");
            sleep $sleepDelay;
        }
    }
    else {
...
###
    ...
    if ($confighash{$cgiparams{'KEY'}}) {
        if (&VPN::ipsecenabled(\%vpnsettings)) {
            ...
            system("/usr/bin/ipsecctrl --stop=$cgiparams{'KEY'}")
        }
                ...
                if ($casubject) { # a new one!
                    ...
                    if ($temp !~ /CA:TRUE/i) {
                        ...
                        if (!$errormessage) {
                            ...
                            system("/usr/bin/ipsecctrl --reload >/dev/null 2>&1");
                        }
                    }
                }
            }
...
;
        ...
        if (&VPN::ipsecenabled(\%vpnsettings)) {
            ...
            system("/usr/bin/ipsecctrl --start=$key >/dev/null 2>&1");
            sleep $sleepDelay;
        }
...
my @status = `/usr/bin/ipsecctrl --status`;
