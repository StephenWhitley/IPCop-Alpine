...
<tr><td style="background-color: #FFFFFF;" align='left'>
    ...
    <a href='#packages'>$Lang::tr{'installed packages'}</a>
</td></tr></table>
END
;
...
if (-e "/etc/mdadm/mdadm.conf") {
    $i = 0;
    open(MDADM, "/usr/bin/sysinfo --raid=md0 |");
    while(<MDADM>) {
        if ($_ =~ m/^\s+(\d+).*\/dev\/(.*)\d$/) {
            if ($i) {
                print "<br /><hr />";
...
foreach my $line (@lines) {
    ...
    print $line."\r";
    (my $slot) = split(/ /, $line);
    $outputnic = `/usr/bin/sysinfo --pci=$slot`;
    $outputnic =~ s/$slot/       /;
    $outputnic = &Header::cleanhtml($outputnic);
    print $outputnic;
}
...
foreach my $line (@lines) {
    ...
    # Get ethtool output and parse for link status summary
    $output = `/usr/bin/sysinfo --link=$iface`;
    my $speed = 'unknown';
    my $duplex = 'unknown';
    my $link = 'no';
    
    foreach my $ethline (split(/\n/, $output)) {
        $speed = $1 if ($ethline =~ /Speed:\s+(.*)/);  
        $duplex = $1 if ($ethline =~ /Duplex:\s+(.*)/);  
        $link = $1 if ($ethline =~ /Link detected:\s+(.*)/);  
    }
    
    $output = sprintf("%s: %s %s link:%s\n", $iface, $speed, $duplex, $link);
...
foreach my $line (@lines) {
    ...
    $output = `/usr/bin/sysinfo --link=$iface | /bin/grep Settings`;
    my $outputethtoolspd = `/usr/bin/sysinfo --link=$iface | /bin/grep Speed`;
    my $outputethtooldup = `/usr/bin/sysinfo --link=$iface | /bin/grep Duplex`;
    my $outputethtoollnk = `/usr/bin/sysinfo --link=$iface | /bin/grep Link`;
    $output = &General::color_devices($output);
...
my $outputusb = `/usr/bin/sysinfo --usb`;
...
# Busybox ps has limited options compared to GNU ps
# Original GNU ps: ps fax -o user,pid,ppid,%cpu,%mem,vsz,rss,tty,stat,start,time,command --cols 128
# Busybox ps doesn't support: f,a,x flags, --cols, -w, or many -o columns
# Use basic -o format without -w flags
my $outputps = `ps -o user,pid,ppid,stat,time,comm`;
...
print "<a name='packages'></a>\n";
&Header::openbox('100%', 'left', "$Lang::tr{'installed packages'}:");
my $outputpkgs = `/sbin/apk info -v -d`;
$outputpkgs = &Header::cleanhtml($outputpkgs);
print "<pre>$outputpkgs</pre>\n";
&Header::closebox();
...
{
    my $dev = shift;
    my $outputhdd = `/usr/bin/sysinfo --disk=$dev`;
