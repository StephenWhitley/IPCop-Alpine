...
# Add entry in menu
# MENU**ENTRY system 080 "backup" "backup"
...
my $setdir       = "$web_root/backup";    # location where sets are stored and imported
my $setdir_download = "/backup";               # URL path for download
...
if ($settings{'ACTION'} eq $Lang::tr{'backup export key'}) {
    if (!defined($settings{'PASSWORD'})) {
        ...
        $errormessage =
            &get_bk_error(system('/usr/bin/ipcopbkcfg', '--keycat', "$settings{'PASSWORD'}", "$tmpfilename") >> 8);
        if (!$errormessage) {
...
if ($settings{'ACTION'} eq $Lang::tr{'create new backup'}) {
    ...
    if (length($settings{'DESCRIPTION'}) > 80) {
        ...
        $errormessage = &get_bk_error(system('/usr/bin/ipcopbkcfg', '--write', "$settings{'DESCRIPTION'}") >> 8);
    }
}
...
if ($settings{'ACTION'} eq $Lang::tr{'import'}) {
    if (blessed($settings{'FH'}) ne 'CGI::File::Temp') {
        ...
        $errormessage = &get_rs_error(system('/usr/bin/ipcoprestore', '--import') >> 8);
    }
}
    ...
    || $settings{'ACTION'} eq $Lang::tr{'restore'})
{
    if ($settings{'ACTION'} eq $Lang::tr{'restore'}) {
        ...
        if ($cryptkeymissing) {
            ...
            $errormessage =
                get_rs_error(system('/usr/bin/ipcoprestore', '--restore', "$settings{'KEY'}", '--hardware') >> 8);
        }
        else {
            $errormessage = get_rs_error(system('/usr/bin/ipcoprestore', '--restore', "$settings{'KEY'}") >> 8);
        }
        if (!$errormessage) {
...
if ($settings{'ACTION'} eq $Lang::tr{'mount'}) {
    if ($settings{'MEDIA'} !~ /^sd[a-z]\d{1,2}$/ && $settings{'DESCRIPTION'} ne '') {
        ...
        system('/usr/bin/ipcopbkcfg', '--umount');
        if ($settings{'MEDIA'} eq $Lang::tr{'local hard disk'}) {
            ...
            $errormessage = `/usr/bin/ipcopbkcfg --mount $settings{'MEDIA'}`;
        }
    }
}
...
if ($settings{'ACTION'} eq $Lang::tr{'backup to floppy'}) {
    print "<hr /><b>$Lang::tr{'alt information'}</b><pre>" . `/usr/bin/ipcopbackup` . '&nbsp;</pre>';
}
print "</td></tr></table>";
