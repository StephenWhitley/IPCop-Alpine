        ...
        foreach my $key (keys %confighash) {
            ...
            if ($test =~ /: OK/) {
                ...
                if (&VPN::ipsecenabled()) {
                    ...
                    system("/usr/bin/ipsecctrl --stop=$key >/dev/null 2>&1");
                }
...
###
    ...
    if (($cgiparams{'GENERATE_ROOT'} eq 'first') && ($cgiparams{'ACTION'} eq $Lang::tr{'generate root/host certificates'})) {
        ...
        my $certdays = int((POSIX::mktime( 0, 0, 1, $cgiparams{'DAY'}, $cgiparams{'MONTH'}, $cgiparams{'YEAR'}-1900) - POSIX::mktime( 0, 0, 0, $now[3], $now[4], $now[5])) / 86400);
        if ($certdays <= 1) {
            $errormessage = $Lang::tr{'invalid date entered'};
            goto ROOTCERT_ERROR;
        }
...
END
        ...
        if (!$errormessage) {
            ...
            $opt .= " -out /var/ipcop/private/dh2048.pem 2048";
            $errormessage = &VPN::callssl ($opt);
        }
...
END
    ...
    # if (&VPN::ipsecenabled) {
    # system('/usr/bin/ipsecctrl', '--start');
    # sleep $sleepDelay;
    # }
