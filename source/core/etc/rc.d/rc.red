...
my %dhcpsettings;
my $iface;
my $dhcp_bin = '/usr/sbin/dhcpcd';
if (! -x $dhcp_bin && -x '/sbin/dhcpcd') { $dhcp_bin = '/sbin/dhcpcd'; }
...
sub dodhcpdial($;$) {
    ...
    # Ensure iface file exists for firewall rules (Essential for puzzleFwRules.pl)
    # We only do this for standard DHCP mode to avoid conflict with PPTP/PPPoE
    if ($netsettings{'RED_1_TYPE'} eq 'DHCP') {
        if (open(FILE, ">/var/ipcop/red/iface")) { print FILE $iface; close FILE; }
        else { &General::log('red', "ERROR: Could not write /var/ipcop/red/iface"); }
    }
    ...
    push(@dhcpcommand, '-b', '-c', '/usr/bin/dhcpcd.sh');
    push(@dhcpcommand, "$iface");

    # Ensure no existing dhcpcd is running on this interface (e.g. from standard Alpine networking)
    system("$dhcp_bin --quiet -k $iface 2>/dev/null");
    system("$dhcp_bin --quiet -k -P /run/dhcpcd-$iface.pid 2>/dev/null");
    system("$dhcp_bin --quiet -k -P /var/run/dhcpcd-$iface.pid 2>/dev/null");

    if (system("@dhcpcommand")) {
        &General::log('red', "dhcp client failed to start: @dhcpcommand");
        myexit(1);
    }
    else {
        &General::log('red', "dhcp client started: @dhcpcommand");
    }
}
...
sub doupdatesettings {
    ...
    # erase in case it was created once with 'persistent' selected but rc.red stop never used : SF1171610
    # BUT do NOT erase it if we are using Ethernet (DHCP/Static) as it's needed for rc.updatered
...
if ($ARGV[0] eq 'start') {
    ...
    if (! system("ps ax | grep -q [r]c.updatered") ) {
        &General::log("ERROR: Can't start RED when rc.updatered is still active");
        exit 1;
    }

    system('touch', '/var/ipcop/red/connecting');
    unlink '/var/ipcop/red/disconnecting';
    if (
        (
    ...
    if ($netsettings{'RED_COUNT'} > 0) {
        if ($netsettings{'RED_1_DEV'} ne '') {
            ...
            if ($netsettings{'RED_1_TYPE'} eq 'STATIC') {
                if (open(FILE, ">/var/ipcop/red/iface")) { print FILE $netsettings{'RED_1_DEV'}; close FILE; }
                system("touch", "/var/ipcop/red/active");
    ...
    if ($pppsettings{'RECONNECTION'} eq 'dialondemand') {
        system('touch', "/var/ipcop/red/dial-on-demand");
    }
    ...
    if (-e "/var/ipcop/red/active") {
        system('touch', '/var/ipcop/red/disconnecting');
    }
    ...
    my $found_pid = 0;
    my $pidfile = "";
    foreach my $p ("/var/run/dhcpcd-$iface.pid", "/run/dhcpcd/$iface.pid", "/run/dhcpcd-$iface.pid") {
        if (-e $p) { $found_pid = 1; $pidfile = $p; last; }
    }
    if ($found_pid) {
    ...
    if (!system("ps -ef | grep -q [a]tmarpd")) {
