...
sub writehash
{
    ...
    # Enforce permissions: root:lighttpd, 0660
    # This ensures files written by root processes (cron, init) are readable by the web server
    my $gid = getgrnam('lighttpd');
    if (defined $gid) {
...
sub isrunning($)
{
    ...
    if ($exename eq 'e2guardian') {
        # Special case for squidguard, no PID file(s) to check.
        my $counter = 0;
        my $output = '';

        $output = `/bin/ps -o vsz= -C e2guardian`;
    ...
    if ($exename eq 'wireguard') {
        # Special case for WireGuard, kernel module, check interface wg0
        if (-d '/sys/class/net/wg0') {
            if ($showsize) {
    ...
    # Special handling for Alpine dcron: PID file is dcron.pid but process name is crond
    if ($exename eq 'dcron') {
        $exename = 'crond';
    }

    # Special handling for rsyslog: PID file name doesn't match process name
    my $pidfile = "/var/run/${cmd}.pid";
    if ($exename eq 'rsyslogd') {
        $pidfile = "/var/run/rsyslog.pid";
    }
    
    # Suricata process is named Suricata-Main
    if ($exename eq 'suricata') {
        $exename = '[Ss]uricata';
    }
...
sub downloadpatch
{
    ...
    if ($databuf && $databuf->is_success) {
        ...
        $ret = system("/usr/bin/installpackage --test=/var/patches/${filename} >/dev/null") >> 8;
...
sub ispatchavailable
{
    ...
    if (ref($available) eq 'HASH' && defined($available->{"latest"}) && ($available->{"latest"} ne ${General::version})) {
...
sub PrintActualLeases
{
    ...
    open(LEASES, "/var/ipcop/dhcp/dnsmasq.leases");
