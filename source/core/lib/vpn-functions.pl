...
sub callssl ($) {
    my $opt = shift;
    # Force use of IPCop's custom OpenSSL configuration which defines [ server ] extensions
    $ENV{'OPENSSL_CONF'} = '/var/ipcop/vpn/openssl.cnf';
    ...
    if ($retval != 0) {
        $ret .= "OpenSSL exit code: $retval<br />";
    }

    foreach my $line (split (/\n/, $retssl)) {
        &General::log("vpn", "$line") if (0);     # 1 for verbose logging
        # Expanded regex to catch more errors including permission denied
        if ( $retval != 0 || $line =~ /error|unknown|denied|failed|warn|unable/i ) {
...
END
    ...
    print CONF "\tleftupdown=/usr/bin/ipsecupdown.sh\n" if ($protostack eq 'netkey');
    print CONF "\n";
...
dh /var/ipcop/private/dh2048.pem
script-security 2
END
    ;
    ...
    print CONF "client-connect /usr/bin/openvpn.sh\n";
    print CONF "client-disconnect /usr/bin/openvpn.sh\n";
    ...
    print CONF "tls-verify /usr/bin/openvpnverify\n";
    print CONF "crl-verify /var/ipcop/crls/cacrl.pem\n";
    print CONF "user nobody\n";
    print CONF "group lighttpd\n";
