...
&General::readhash('/var/ipcop/ethernet/settings', \%settings);
...
require '/usr/lib/ipcop/ipcop_paths.pl'; # Assuming installed here or use relative
# Fallback if module not found during build
my $web_root = $IPCop::Paths::IPCOP_WEB_ROOT || "/var/www/ipcop/html";
my $graphs = "$web_root/graphs";
...
sub rrd_header {
    ...
    push @$result, ("--width", 1000);
    push @$result, ("--height", 200);
    if ($lower != -1) {
    ...
    push @$result, ("--font", "DEFAULT:10:monospace");
    push @$result, ("--font", "TITLE:12:monospace");
    push @$result, ("--font", "LEGEND:10:monospace");
...
sub updatecpugraph {
    ...
    push @rrd, "AREA:systempct#FF0000:$Lang::tr{'system cpu usage'}" . (" " x ($col_width - length($Lang::tr{'system cpu usage'}))) . ":STACK";
    ...
    push @rrd, "AREA:idlepct#00FF00:$Lang::tr{'idle cpu usage'}" . (" " x ($col_width - length($Lang::tr{'idle cpu usage'}))) . ":STACK";
...
sub updatememgraph {
    ...
    push @rrd, "STACK:bufferpct#FF00FF:$Lang::tr{'buffered memory'}" . (" " x ($col_width - length($Lang::tr{'buffered memory'})));
    ...
    push @rrd, "STACK:swapcachedpct#FF0000:$Lang::tr{'swapcached memory'}" . (" " x ($col_width - length($Lang::tr{'swapcached memory'})));
...
sub updatediskgraph {
    ...
    my $lbl_read = "$Lang::tr{'sectors read from disk per second'}";
    my $lbl_write = "$Lang::tr{'sectors written to disk per second'}";
    my $col_width = (length($lbl_read) > length($lbl_write)) ? length($lbl_read) : length($lbl_write);
    $col_width += 2;

    push @rrd, "AREA:readneg#0000FF:$lbl_read" . (" " x ($col_width - length($lbl_read))) . "\\j";
    ...
    push @rrd, "AREA:write#00FF00:$lbl_write" . (" " x ($col_width - length($lbl_write))) . "\\j";
...
sub updateifgraph {
    ...
    my $lbl_in = "$Lang::tr{'incoming traffic in bits per second'}";
    my $lbl_out = "$Lang::tr{'outgoing traffic in bits per second'}";
    my $col_width = (length($lbl_in) > length($lbl_out)) ? length($lbl_in) : length($lbl_out);
    $col_width += 2; # Alignment padding

    push @rrd, "HRULE:0#000000";
    push @rrd, "AREA:incomingbits#00FF00:$lbl_in" . (" " x ($col_width - length($lbl_in)));
    ...
    push @rrd, "AREA:outgoingnegbits#0000FF:$lbl_out" . (" " x ($col_width - length($lbl_out)));
...
sub updateifdata {
    ...
    if (!-e "$rrdlog/$interface.rrd") {
        RRDs::create(
            ...
            "DS:incoming:DERIVE:600:0:U",  "DS:outgoing:DERIVE:600:0:U",
            "RRA:AVERAGE:0.5:1:576",       "RRA:AVERAGE:0.5:6:672",
            "RRA:AVERAGE:0.5:24:732",      "RRA:AVERAGE:0.5:144:1460"
        );
        $ERROR = RRDs::error;
...
sub updatesquiddata {
    ...
    # Create squid directory in collectd structure
    my $squid_dir = "$rrdlog/localhost/squid";
    if (!-d $squid_dir) {
        mkdir $squid_dir or warn "Could not create $squid_dir: $!";
        chmod 0755, $squid_dir;
    }
    
    my $squid_rrd = "$squid_dir/squid_requests.rrd";
    if (!-e $squid_rrd) {
        RRDs::create(
            $squid_rrd,
            "--step=10",  # 10 seconds to match collectd granularity
    ...
    $sock->autoflush(1);
    # Squid 7.x uses /squid-internal-mgr/info instead of cache_object://
    print $sock "GET /squid-internal-mgr/info HTTP/1.0\r\nHost: $host\r\n\r\n";
    my @result = <$sock>;
    close $sock;
    ...
    # Default to 0 if values not found (e.g., new cache with no hits yet)
    $vals{requests} = 0 unless defined($vals{requests});
    $vals{hits_per} = 0 unless defined($vals{hits_per});

#    print "$vals{requests} $vals{hits_per}\n";

    RRDs::update("$squid_rrd", "-t", "requests:hits_per", "N:$vals{requests}:$vals{hits_per}");
    $ERROR = RRDs::error;
...
updatesquiddata();
# PNG graph generation removed - GraphsJS handles all visualization
