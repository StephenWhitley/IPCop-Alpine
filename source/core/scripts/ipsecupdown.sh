...
case "$PLUTO_VERB" in
    ...
    up-client)
        ...
        /usr/sbin/iptables -t nat -I REDNAT -s ${PLUTO_MY_CLIENT} -d ${PLUTO_PEER_CLIENT} -j ACCEPT
        ;;
    down-client)
        /usr/bin/logger -t ipsec "$PLUTO_VERB"
        # connection to my client subnet going down
        /usr/sbin/iptables -t nat -D REDNAT -s ${PLUTO_MY_CLIENT} -d ${PLUTO_PEER_CLIENT} -j ACCEPT
        ;;
    *)
