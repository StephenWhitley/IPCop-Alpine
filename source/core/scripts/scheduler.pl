...
sub dial
{
    ...
    unless (system('rc-service', 'ipcop-red', 'start') == 0) {
...
sub hangup
{
    ...
    unless (system('rc-service', 'ipcop-red', 'stop') == 0) {
...
sub blacklistupdate
{
    ...
    system("/usr/bin/blacklistupdate.pl --force");
}
...
sub dyndns
{
    ...
    system("/usr/bin/setddns.pl --force");
}

sub update
{
    system("/usr/bin/updatelists.pl --cron");
}

sub reboot
{
    system("/usr/bin/ipcopreboot --boot Scheduled reboot");
}

sub shutdown
{
    system("/usr/bin/ipcopreboot --down Scheduled shutdown");
}
...
sub ipsec
{
    ...
    system("/usr/bin/ipsecctrl --${parameter}");
}
...
sub openvpn
{
    ...
    system("/usr/bin/restartopenvpn --${parameter}");
}
