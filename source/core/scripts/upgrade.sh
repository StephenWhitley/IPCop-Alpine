...
if [ -e /var/ipcop/openvpn/server.conf ]; then
    ...
    /bin/sed -i -e "s+^client-connect.*+client-connect /usr/bin/openvpn.sh+" \
                -e "s+^client-disconnect.*+client-disconnect /usr/bin/openvpn.sh+" \
...
chown root:lighttpd /var/ipcop/proxy
chmod 0775 /var/ipcop/proxy
