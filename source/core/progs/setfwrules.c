...
int main(int argc, char *argv[])
{
    ...
    if (flag_cron) {
        snprintf(command, STRING_SIZE, "/usr/bin/puzzleFwRules.pl %s -c", opt_debug);
        safe_system(command);
    }

    if (flag_all) {
        snprintf(command, STRING_SIZE, "/usr/bin/puzzleFwRules.pl %s -a", opt_debug);
        safe_system(command);
    }

    if (flag_force) {
        snprintf(command, STRING_SIZE, "/usr/bin/puzzleFwRules.pl %s -f %s", opt_debug, opt_chain);
        safe_system(command);
    }

    if (flag_ipcop) {
        snprintf(command, STRING_SIZE, "/usr/bin/puzzleFwRules.pl %s -i", opt_debug);
        safe_system(command);
    }

    if (flag_user) {
        snprintf(command, STRING_SIZE, "/usr/bin/puzzleFwRules.pl %s -u", opt_debug);
        safe_system(command);
    }

    if (flag_wireless) {
        snprintf(command, STRING_SIZE, "/usr/bin/puzzleFwRules.pl %s -w", opt_debug);
        safe_system(command);
    }
