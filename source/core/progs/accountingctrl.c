/*
 * accountingctrl.c: Simple program to setup iptables rules for traffic
 * accounting.
 *
...
static void add_device_rules(char *dev_id, char *iface) {
  ...
  snprintf(options, STRING_SIZE,
           "-j NFLOG --nflog-group 1 --nflog-threshold 50 --nflog-range 48");

  verbose_printf(2, "  For %s:\n", dev_id);

  snprintf(
      command, STRING_SIZE_LARGE,
      "/usr/sbin/iptables -A ACCOUNT_INPUT -i %s %s --nflog-prefix %s_INPUT",
      iface, options, dev_id);
  verbose_printf(2, "    %s\n", command);
  safe_system(command);

  snprintf(command, STRING_SIZE_LARGE,
           "/usr/sbin/iptables -A ACCOUNT_FORWARD_IN -i %s %s "
           "--nflog-prefix %s_FORWARD_IN",
           iface, options, dev_id);
  verbose_printf(2, "    %s\n", command);
  safe_system(command);

  snprintf(command, STRING_SIZE_LARGE,
           "/usr/sbin/iptables -A ACCOUNT_FORWARD_OUT -o %s %s "
           "--nflog-prefix %s_FORWARD_OUT",
           iface, options, dev_id);
  verbose_printf(2, "    %s\n", command);
  safe_system(command);

  snprintf(command, STRING_SIZE_LARGE,
           "/usr/sbin/iptables -A ACCOUNT_OUTPUT -o %s %s --nflog-prefix "
           "%s_OUTPUT",
           iface, options, dev_id);
  verbose_printf(2, "    %s\n", command);
  safe_system(command);
...
static void add_vnstat_db(char *dev_id, char *iface) {
  ...
  if (access(command, 0) == -1) {
    snprintf(message, STRING_SIZE, "Create vnstat DB for %s nick %s", iface,
             dev_id);
    verbose_printf(1, "%s\n", message);
     ...
     */
    snprintf(command, STRING_SIZE_LARGE,
             "/usr/bin/vnstat --update --iface %s --nick %s > /dev/null", iface,
             dev_id);
    safe_system(command);

    /* Stop vnstatd, later code will notice that PID file is gone and restart
     * vnstatd */
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "chv", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 0:
      break;
  ...
  if (flag_empty_ulogd || flag_empty_all) {
    ...
    if (flag_empty_all) {
      ...
      safe_system("cp /var/ipcop/traffic/empty-aggregate.db "
                  "/var/log/traffic/aggregate.db");
  ...
  safe_system("/usr/sbin/iptables -F ACCOUNT_INPUT\n");
  safe_system("/usr/sbin/iptables -F ACCOUNT_FORWARD_IN\n");
  safe_system("/usr/sbin/iptables -F ACCOUNT_FORWARD_OUT\n");
  safe_system("/usr/sbin/iptables -F ACCOUNT_OUTPUT");
  ...
  if (enabled) {
    ...
    if ((ipcop_ethernet.count[RED] == 0) &&
        (strlen(ipcop_ethernet.red_device[1]))) {
    ...
    if (detail_high && safe_system("/usr/bin/pgrep ulogd > /dev/null")) {
      verbose_printf(1, "Start ulogd ... \n");
      safe_system("/usr/sbin/ulogd -d");
    }
  ...
  if (!safe_system("/usr/bin/pgrep ulogd > /dev/null") &&
      !(enabled && detail_high)) {
    verbose_printf(1, "Stop ulogd ... \n");
    safe_system("/usr/bin/killall ulogd");
  }
