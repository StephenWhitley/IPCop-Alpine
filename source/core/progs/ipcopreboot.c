...
int main(int argc, char**argv)
{
    ...
    if (flag_down) {
        ...
        /* Use Alpine's poweroff command with background delay */
        safe_system("(sleep 10; /sbin/poweroff) &");
        return 0;
    }
    if (flag_boot) {
        syslog(LOG_NOTICE, "IPCop will shutdown and reboot");
        verbose_printf(1, "IPCop will shutdown and reboot ... \n");
        /* Use Alpine's reboot command with background delay */
        safe_system("(sleep 10; /sbin/reboot) &");
        return 0;
    }
    if (flag_bootfs) {
        ...
        /* Create force fsck flag file, then reboot with delay */
        safe_system("/usr/bin/touch /forcefsck");
        safe_system("(sleep 10; /sbin/reboot) &");
        return 0;
    }
    closelog();
