...
static void ourexit(int err) {
  ...
  if (err) {
    ...
    case ERR_TAR:
      snprintf(command, STRING_SIZE,
               "/usr/bin/logger -t installpackage \"ERROR (%d): This is not a "
               "valid archive.\"",
               err);
      break;
    case ERR_INFO:
      snprintf(command, STRING_SIZE,
               ...
               "update information file. The update file is corrupt.\"",
               err);
      break;
    case ERR_PACKLIST:
      snprintf(command, STRING_SIZE,
               "/usr/bin/logger -t installpackage \"ERROR (%d): Could not open "
               "installed updates file.\"",
               err);
      break;
    case ERR_INSTALLED:
      snprintf(command, STRING_SIZE,
    ...
    case ERR_MISSING_PREVIOUS:
      ...
      snprintf(command, STRING_SIZE,
               ...
               "ERR_MISSING_PREVIOUS.\"",
               err);
      break;
    case ERR_DISK:
      snprintf(command, STRING_SIZE,
               "/usr/bin/logger -t installpackage \"ERROR (%d): Not enough "
               "disk space.\"",
               err);
      break;
    case ERR_SUID:
...
static void usage(char *prg, int exit_code) {
  ...
  printf("Options:\n");
  printf("  --install=FILENAME    Decrypt FILENAME and install upgrade (only "
         "if valid)\n");
  printf(
      "  --test=FILENAME       Decrypt FILENAME and test if valid upgrade\n");
...
void main(int argc, char *argv[]) {
  ...
  static struct option long_options[] = {{"install", required_argument, 0, 'i'},
  ...
  if (rootspacerequired > spaceavailable) {
    ...
    fprintf(stderr,
            ...
            spaceavailable / 1024, rootspacerequired / 1024);
    ourexit(ERR_DISK);
  }
  ...
  /* Process is :
     ...
     - file is untarred inside tmp dir to patch.tar.gz, information.xml and
     setup
  ...
  snprintf(command, STRING_SIZE, "/bin/grep '^gpg' -m 2 %s/signature >%s",
           tmpdir, signature_filename);
  safe_system(command);
  snprintf(command, STRING_SIZE, "/bin/chown nobody:nobody %s",
           signature_filename);
  safe_system(command);

  /* fetch information, needed for allowing offline updates */
  snprintf(command, STRING_SIZE,
           "/bin/tar xzf %s/patch-1.tgz -C /var/patches information.xml",
           tmpdir);
  if (safe_system(command)) {
  ...
  if (flag_test) {
    ...
    snprintf(
        command, STRING_SIZE,
        "/usr/bin/perl -e \"require '/usr/lib/ipcop/general-functions.pl'; "
        "&General::updateavailablepatches('/var/patches/information.xml');\"");
    safe_system(command);
  ...
  snprintf(command, STRING_SIZE,
           "/usr/bin/perl -e \"require '/usr/lib/ipcop/general-functions.pl'; "
           "&General::updateinstalledpatches('%s/information.xml');\"",
           tmpdir);
  safe_system(command);
