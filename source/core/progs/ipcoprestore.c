...
void usage(char *prg, int exit_code) {
  ...
  fprintf(stderr, "  --import              rename <hostname>.dat to "
                  "<hostname>.YYYY-MM-DD_HH-MM-SS.dat\n");
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "r:v", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 0:
  ...
  if (flag_restore) {
    ...
    if (strlen(opt_filename) !=
        (strlen(MOUNTPOINT "/-YYYY-MM-DD_HH-MM-SS.dat") + strlen(hostname))) {
  ...
  snprintf(command, STRING_SIZE - 1, "/bin/tar -C %s -xzvf /tmp/%s.tar.gz",
           tmpdir, hostname);
  if (safe_system(command)) {
  ...
  if (flag_import) {
    ...
    if (access(filename, F_OK) == -1) {
      ...
      snprintf(command, STRING_SIZE - 1,
               ...
               "/usr/bin/awk '{print $6}' | "
               "/usr/bin/sort -rn | /bin/head -n 1 >%s",
               tmpdir, tmpdatefile);
      if (safe_system(command)) {
      ...
      snprintf(filename, STRING_SIZE - 1, MOUNTPOINT "/%s-%s.dat.time",
               hostname, timestamp);
      ...
      snprintf(command, STRING_SIZE - 1,
               "/bin/mv %s " MOUNTPOINT "/%s-%s.dat.time", filename, hostname,
               timestamp);
      if (safe_system(command)) {
    ...
    snprintf(filename, STRING_SIZE - 1, MOUNTPOINT "/%s.dat", hostname);
    snprintf(command, STRING_SIZE - 1, "/bin/mv %s " MOUNTPOINT "/%s-%s.dat",
  ...
  /* uncompress archive */
  snprintf(command, STRING_SIZE - 1, "/bin/gunzip -d -f /tmp/%s.tar.gz",
           hostname);
  safe_system(command);
   ...
   * Use -X to exclude both system files and hardware files (if flag_hardware ==
   * 0) BusyBox tar doesn't support --delete, so we exclude during extraction
   * instead
   */
  if (flag_hardware == 0) {
    snprintf(command, STRING_SIZE - 1,
             "/bin/tar -X /var/ipcop/backup/exclude.system "
             "-X /var/ipcop/backup/exclude.hardware "
             "-C / -xvf /tmp/%s.tar",
             hostname);
  } else {
    snprintf(
        command, STRING_SIZE - 1,
        "/bin/tar -X /var/ipcop/backup/exclude.system -C / -xvf /tmp/%s.tar",
        hostname);
  }
  if (safe_system(command)) {
    ...
    /* TODO: here we will need to add mechanism for upgrade from 1.4.xx
     * configuration files */
    ...
    if (read_kv_from_file(&kv_new, "/var/ipcop/main/settings") == SUCCESS) {
      if (test_kv(kv_new, "GUIPORT", port_gui) != SUCCESS) {
        ...
        snprintf(command, STRING_SIZE,
                 "/usr/bin/setreservedports.pl --nocheck --gui %s", port_gui);
        safe_system(command);
      }
      if (test_kv(kv_new, "SSHPORT", port_ssh) != SUCCESS) {
        find_kv_default(kv_new, "SSHPORT", port_ssh);
        snprintf(command, STRING_SIZE,
                 "/usr/bin/setreservedports.pl --nocheck --ssh %s", port_ssh);
        safe_system(command);
      }
    ...
    if (safe_system("/usr/bin/upgrade.sh")) {
