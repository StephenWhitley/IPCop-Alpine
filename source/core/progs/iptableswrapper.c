...
int main(int argc, char *argv[]) {
  ...
  if ((argc == 2) &&
      (!strcmp("filter", argv[1]) || !strcmp("mangle", argv[1]) ||
       !strcmp("nat", argv[1]) || !strcmp("raw", argv[1]))) {
    snprintf(command, BUFFER_SIZE - 1,
             "/usr/sbin/iptables -t %s --line-numbers -nvL", argv[1]);
  } else if ((argc == 4) && !strcmp("chain", argv[1])) {
    snprintf(command, BUFFER_SIZE - 1,
             "/usr/sbin/iptables -t %s --line-numbers -nvL %s", argv[2],
             argv[3]);
  }
