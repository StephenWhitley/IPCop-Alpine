...
#define EXCLUDE_HARDWARE                                                       \
  "exclude.hardware" // exclude file not used on backup but only optionally on
                     // restore
/* #define MOUNTPOINT "/var/www/ipcop/html/backup" - Now using common define */
/* MOUNTPOINT now constructed using IPCOP_WEB_ROOT in main or defined here */
#define MOUNTPOINT IPCOP_WEB_ROOT "/backup"
...
int cmb_files(char *outfilename, const char *outfname, const char *fnamebase,
              const int verbose) {
  ...
  if (namecount < 0) {
    ...
    while (namecount--) {
      ...
      if (strncasecmp(fnamebase, namelist[namecount]->d_name,
                      strlen(fnamebase)) == 0) {
        /* hardware settings need not to be exclude on backup but optionally on
         * restore */
        if (strncmp(namelist[namecount]->d_name, EXCLUDE_HARDWARE,
                    strlen(EXCLUDE_HARDWARE))) {
          /* add the contents for this name to output file */
          sprintf(addfilename, "/var/ipcop/backup/%s",
...
static void usage() {
  ...
  fprintf(stderr, "ipcopbkcfg --keygen\n");
  fprintf(stderr, "\tGenerate a new backup.key if one doesn't exist\n");
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    if (!(strstr(argv[3], "/tmp/logfile.") &&
          strlen(argv[3]) == strlen(TMPLOGFILE))) {
    ...
    /* Generate a new backup key if one doesn't exist */
    if (file_exists(BACKUP_KEY)) {
      fprintf(stderr, "Backup key already exists\n");
      exit(1);
    }
    /* Create backup directory if needed */
    snprintf(command, STRING_SIZE - 1, "/bin/mkdir -p /var/ipcop/backup");
    safe_system(command);
    /* Generate 256-bit random key using openssl */
    snprintf(command, STRING_SIZE - 1,
             "/usr/bin/openssl rand -base64 32 > " BACKUP_KEY);
    if (safe_system(command)) {
      fprintf(stderr, "Failed to generate backup key\n");
      exit(1);
    }
    /* Set proper permissions (readable only by root) */
    chmod(BACKUP_KEY, 0400);
    chown(BACKUP_KEY, 0, 0);
    exit(0);
  } else if (argc == 3 && strcmp(argv[1], "--mount") == 0 &&
             strlen(argv[2]) > 2 && strlen(argv[2]) < 6 &&
             strspn(argv[2], LETTERS_NUMBERS) == strlen(argv[2])) {
    ...
    snprintf(command, STRING_SIZE - 1,
             "/bin/mount -t vfat -o,uid=99,gid=99 /dev/%s " MOUNTPOINT,
             argv[2]);
  ...
  cmb_files(tempincfilename, "/tmp/backup-inclusion.XXXXXX", "include.",
            verbose);
  cmb_files(tempexcfilename, "/tmp/backup-exclusion.XXXXXX", "exclude.",
            verbose);
  ...
  FILE *versionfile = fopen("/var/ipcop/backup/version", "w");
  if (!versionfile) {
   ...
   * Note: -W (verify) removed for busybox tar compatibility */
  snprintf(command, STRING_SIZE - 1, "/bin/tar -T %s -X %s -C / -cf %s",
           tempincfilename, tempexcfilename, temptarfile);

  /* Debug output: show who is running and what command */
  fprintf(stderr, "[DEBUG] Running as UID=%d EUID=%d GID=%d EGID=%d\n",
          getuid(), geteuid(), getgid(), getegid());
  fprintf(stderr, "[DEBUG] Executing: %s\n", command);
  fprintf(stderr, "[DEBUG] Temp tarfile: %s\n", temptarfile);

  int tar_result = safe_system(command);
  if (tar_result) {
    ...
    fprintf(
        stderr,
        "ERROR: Check if /tmp is writable and tar has proper permissions\n");
    exit(BACKUP_ERR_TAR);
  }

  /* Compress archive */
  snprintf(command, STRING_SIZE - 1, "/bin/gzip -c < %s > %s", temptarfile,
           temptgzfile);
  if (safe_system(command)) {
  ...
  snprintf(command, STRING_SIZE - 1,
           ...
           "-out " MOUNTPOINT "/%s-%s.dat "
           "-kfile " BACKUP_KEY,
           temptgzfile, hostname, timestamp);
  if (safe_system(command)) {
  ...
  snprintf(command, STRING_SIZE - 1, MOUNTPOINT "/%s-%s.dat.time", hostname,
           timestamp);
  writetagfile(command, timestamp, argv[2]);

  /* Make sure web can overwrite */
  snprintf(command, STRING_SIZE - 1, MOUNTPOINT "/%s-%s.dat", hostname,
           timestamp);
