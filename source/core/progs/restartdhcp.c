...
int main(int argc, char **argv) {
  ...
  static struct option long_options[] = {{"boot", no_argument, 0, 'b'},
  ...
  while ((c = getopt_long(argc, argv, "bsv", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 'b': /* booting */
      flag_boot = 1;
  ...
  for (i = 0; i < CFG_COLOURS_COUNT; i++) {
    /* filter all but GREEN, BLUE and ORANGE */
    if ((i != GREEN) && (i != BLUE) && (i != ORANGE)) {
      continue;
    }
  ...
  if (flag_sighup) {
    ...
    safe_system("/usr/bin/killall -HUP dnsmasq");
  } else if (!flag_boot) {
    verbose_printf(1, "Restarting dnsmasq ... \n");
    safe_system("/sbin/rc-service dnsmasq restart");
  }
