...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "frv", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 'r': /* red update */
      flag_red = 1;
  ...
  if (access("/var/run/ntpd.pid", 0) != -1) {
    ...
    safe_system("/usr/sbin/iptables -t nat -F NTP");
  ...
  if (enabled && (flag_red || flag_quick)) {
    ...
    if (!flag_verbose) {
      /* Silence ntpdate, even with -s it can still produce messages in case RED
       * down */
    ...
    if (flag_red && rc) {
      /* Add some output to indicate ntpdate failed and we possibly have wrong
       * time */
  ...
  if (enabled && enabled_redirect) {
    ...
    for (i = 1; i <= ipcop_ethernet.count[GREEN]; i++) {
      ...
      snprintf(command, STRING_SIZE - 1,
               "/usr/sbin/iptables -t nat -A NTP -i %s -p udp --dport 123 ! "
               "-d %s -j DNAT --to %s",
               ipcop_ethernet.device[GREEN][i],
               ipcop_ethernet.address[GREEN][i],
               ipcop_ethernet.address[GREEN][i]);
      safe_system(command);
    }
    /* redirect for BLUE */
    for (i = 1; i <= ipcop_ethernet.count[BLUE]; i++) {
      ...
      snprintf(command, STRING_SIZE - 1,
               "/usr/sbin/iptables -t nat -A NTP -i %s -p udp --dport 123 ! "
               "-d %s -j DNAT --to %s",
               ipcop_ethernet.device[BLUE][i], ipcop_ethernet.address[BLUE][i],
               ipcop_ethernet.address[BLUE][i]);
      safe_system(command);
    }
  }
