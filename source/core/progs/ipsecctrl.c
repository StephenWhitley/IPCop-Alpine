...
int main(int argc, char *argv[])
{
    ...
    if (flag_start && ((connection == NULL) || (ipsec_running() == FAILURE))) {
        ...
        if (flag_verbose) {
            ...
            safe_system("rc-service ipsec restart");
        }
        else {
            if (safe_system("/sbin/modinfo ipsec 2>/dev/null") == 0) {
                /* KLIPS only */
            ...
            safe_system("rc-service ipsec restart >/dev/null");
        }
        add_alias_interfaces((enable_red + enable_blue) >> 1);
        if (connection == NULL) {
            safe_system("/usr/bin/vpn-watch --start");
            exit(0);
        }
    }
