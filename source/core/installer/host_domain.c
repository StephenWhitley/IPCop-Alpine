...
static int writehostsfiles(char *hostname, char *domainname) {
  ...
  if (flag_is_state == setupchroot) {
    /* rebuildhosts will send SIGHUP to dnsmasq, not a problem as dnsmasq is not
     * there (yet) */
    mysystem("/usr/bin/rebuildhosts");
  } else {
    /* restart dnsmasq so it knows about new host/domain */
    mysystem("/usr/bin/rebuildhosts --nosighup");
  }
  ...
  /* Set proper ownership for web interface access */
  struct group *gr = getgrnam("lighttpd");
  if (gr) {
    fchown(fileno(file), 0, gr->gr_gid);
  }
  fchmod(fileno(file), 0660);
  fprintf(file, "ServerName %s.%s\n", hostname, domainname);
  fclose(file);

  /* Write to /etc/hostname for Alpine Linux persistence */
  /* Use direct file I/O instead of shell command to avoid mysystem() issues */
  {
    FILE *hn_file = fopen("/etc/hostname", "w");
    if (hn_file) {
      fprintf(hn_file, "%s.%s\n", hostname, domainname);
      fclose(hn_file);
    }
  ...
  /* Apply hostname immediately using sethostname() syscall */
  /* This is more reliable than spawning a shell */
  {
    char fullhostname[STRING_SIZE];
    snprintf(fullhostname, STRING_SIZE, "%s.%s", hostname, domainname);
    if (sethostname(fullhostname, strlen(fullhostname)) != 0) {
      /* sethostname failed (needs root), try shell fallback */
      snprintf(
          commandstring, STRING_SIZE,
          "hostname -F /etc/hostname 2>/dev/null || hostname '%s' 2>/dev/null",
          fullhostname);
...
int handlehostname(void) {
  ...
  for (;;) {
    rc = newtWinEntries(gettext("TR_HOSTNAME"), gettext("TR_ENTER_HOSTNAME"),
                        ...
                        (flag_is_state == setupchroot) ? gettext("TR_SKIP")
                                                       : gettext("TR_GO_BACK"),
                        NULL);
    ...
    if (rc == 1) {
      ...
      if (!(strlen(hostname)))
        ...
        errorbox(gettext("TR_HOSTNAME_CANNOT_CONTAIN_SPACES"));
               ...
               strspn(hostname, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRST"
                                "UVWXYZ0123456789-"))
        ...
        if (flag_is_state == setup) {
          /* In case of installation we will simply rewrite the files after
...
int handledomainname(void) {
  ...
  for (;;) {
    rc =
        newtWinEntries(gettext("TR_DOMAINNAME"), gettext("TR_ENTER_DOMAINNAME"),
                       65, 5, 5, 40, entries, gettext("TR_OK"),
                       (flag_is_state == setupchroot) ? gettext("TR_SKIP")
                                                      : gettext("TR_GO_BACK"),
                       NULL);
    ...
    if (rc == 1) {
      ...
      else if (strlen(domainname) !=
               strspn(domainname, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQR"
                                  "STUVWXYZ0123456789-."))
      ...
      result = FAILURE;
      if (flag_is_state == setupchroot) {
        /* In case of installation always write the files, so they exist
         * afterwards */
        writehostsfiles(hostname, domainname);
      }
      break;
