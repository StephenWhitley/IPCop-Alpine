...
/* Define mac_buffer here (declared extern in common.h) */
char mac_buffer[STRING_SIZE];

/* use ---- for non-assigned card */
char *ipcop_colours_text[CFG_COLOURS_COUNT] = {"GREEN", "RED", "BLUE", "ORANGE",
                                               "----"};
char *ipcop_aliases_text[CFG_COLOURS_COUNT] = {"lan", "wan", "wlan", "dmz",
                                               "unused"};

char *ipcop_red_text[CFG_RED_COUNT] = {"ANALOG", "GSM3G",  "ISDN", "PPPOE",
                                       "PPTP",   "STATIC", "DHCP"};
...
int mysystem(char *command) {
  char mycommand[STRING_SIZE_LARGE +
                 64]; // we have a large buffer for the initrd setup!
  int cr = 0;
  int result;
  ...
  /* Run command - if logging, log output to a temp file then append to log */
  /* Previously this was broken: used FILE* as %s which created garbage commands
   */
  if (flog != NULL) {
    fprintf(flog, "Running command: %s\n", command);
    if (cr)
      fprintf(flog, "WARNING mysystem return value is always 0 when a CR is on "
                    "command\n");
    fflush(flog);
  }
...
int mysystemhidden(char *command) {
  char mycommand[STRING_SIZE_LARGE + 64];

  snprintf(mycommand, STRING_SIZE_LARGE, "%s >> /dev/null", command);
  if (flog != NULL) {
    fprintf(flog, "Running command: %s\n", "HIDDEN");
  }
...
int write_kv_to_file(NODEKV **p, char *filename) {
  ...
  /* Enforce ownership root:lighttpd and perms 0660 for web interface access */
  struct group *gr = getgrnam("lighttpd");
  if (gr) {
    if (fchown(fileno(file), 0, gr->gr_gid) != 0) {
      /* ignore error, just try */
    }
  }
  fchmod(fileno(file), 0660);
...
static int read_ethernet_key(int colour, int index, char *eth_key, char **ptr,
                             int exitonerror, char *error_description) {
  char key[STRING_SIZE];

  /* Build the key, for example GREEN_1_ADDRESS */
  snprintf(key, STRING_SIZE, "%s_%d_%s", ipcop_colours_text[colour], index,
           eth_key);
  if (find_kv(eth_kv, key) == NULL) {
    ...
    if (exitonerror) {
      ...
      fprintf(stderr, "%s for %s_%d not defined\n", error_description,
              ipcop_colours_text[colour], index);
      exit(1);
    }
  ...
  *ptr = strdup(find_kv(eth_kv, key));
  verbose_printf(2, "  %s for %s_%d: %s\n", error_description,
                 ipcop_colours_text[colour], index, *ptr);
  return SUCCESS;
}
...
int read_ethernet_settings(int exitonerror) {
  ...
  for (i = 0; i < NONE; i++) {
    ...
    if ((ipcop_ethernet.count[i] < 0) ||
        (ipcop_ethernet.count[i] > MAX_NETWORK_COLOUR)) {
    ...
    for (j = 1; j <= ipcop_ethernet.count[i]; j++) {
      if (read_ethernet_key(i, j, "DEV", &ipcop_ethernet.device[i][j],
                            exitonerror, "Device") != SUCCESS)
        return FAILURE;
      if (read_ethernet_key(i, j, "ADDRESS", &ipcop_ethernet.address[i][j],
                            exitonerror, "IP address") != SUCCESS)
        return FAILURE;
      if (read_ethernet_key(i, j, "NETMASK", &ipcop_ethernet.netmask[i][j],
                            exitonerror, "Subnetmask") != SUCCESS)
        return FAILURE;
      if (read_ethernet_key(i, j, "NETADDRESS",
                            &ipcop_ethernet.netaddress[i][j], exitonerror,
                            "Network address") != SUCCESS)
        return FAILURE;

      read_ethernet_key(i, j, "DRIVER", &ipcop_ethernet.driver[i][j], 0,
                        "Driver");
    }
  }

  /* We need at least 1 RED connection type */
  if (read_ethernet_key(RED, 1, "TYPE", &ipcop_ethernet.red_type[1],
                        exitonerror, "Connection type") != SUCCESS)
    return FAILURE;
  /* more red types, we may one day allow for more red devices */
  for (j = 2; j <= ipcop_ethernet.count[RED]; j++) {
    if (read_ethernet_key(RED, j, "TYPE", &ipcop_ethernet.red_type[j],
                          exitonerror, "Type") != SUCCESS)
      return FAILURE;
  }
...
int VALID_IP(const char *ip) {
  ...
  if ((strlen(ip) < 7) || (strlen(ip) > 15))
    return FALSE;
  if (strspn(ip, NUMBERS ".") != strlen(ip))
    return FALSE;
