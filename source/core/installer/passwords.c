...
/* Moved common_newt.h to the end to prevent reordering issues */
...
static int getpassword(char *password, char *text) {
  ...
  do {
    ...
    rc = newtWinEntries(gettext("TR_TITLE_PASSWORD"), text, 68, 5, 5, 40,
                        entries, gettext("TR_OK"),
                        (flag_is_state == setup) ? gettext("TR_GO_BACK") : NULL,
                        NULL);
...
int password(char *user) {
  ...
  if (!strcmp(user, "root")) {
    ...
    /* workaround for potential format string vulnerability if gettext returns
     * mismatching args */
    /* Fallback to English if dangerous, or simpler construction */
    snprintf(message, STRING_SIZE,
             "Enter %s 'admin' user password (web interface).", NAME);
  ...
  if (getpassword(password, message) == SUCCESS) {
    if (!strcmp(user, "admin") || !strcmp(user, "dial")) {
      snprintf(commandstring, STRING_SIZE,
               "/usr/bin/htpasswd -m -b /var/ipcop/auth/users %s '%s'", user,
               password);
    } else {
      snprintf(commandstring, STRING_SIZE,
               "/bin/echo '%s:%s' | /usr/sbin/chpasswd", user, password);
    }

    if (mysystemhidden(commandstring)) {
      snprintf(message, STRING_SIZE, "%s %s",
               gettext("TR_PROBLEM_SETTING_PASSWORD_FOR"), user);
...
int handlepasswords(void) {
  ...
  for (;;) {
    ...
    rc = newtWinMenu(gettext("TR_PASSWORDS"), gettext("TR_SELECT_THE_ITEM"), 50,
                     5, 5, 11, menuchoices, &choice, gettext("TR_OK"),
