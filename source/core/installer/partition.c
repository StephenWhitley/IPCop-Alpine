...
#define FORMAT_FSTAB  "%-14s %-14s %-10s %-20s %-5s %-5s\n"
    ...
    */
    ...
    f = (newtComponent *) statuswindow_progress(72, 5, ipcop_gettext("TR_TITLE_DISK"),
                                                "Installing IPCop packages...");
    
    fprintf(flog, "=== Starting APK package installation ===\n");
    
    /* Step 1: Initialize APK database on target system */
    fprintf(flog, "Initializing APK database...\n");
    if (apk_init_db("/harddisk") != SUCCESS) {
        newtFormDestroy(*f);
        newtPopWindow();
        errorbox("Failed to initialize APK package database");
        return FAILURE;
    }
    
    /* Step 2: Mount bundled repository (for CDROM installs) or configure network repo */
    if (medium_sources == cdrom) {
        fprintf(flog, "Mounting bundled IPCop repository...\n");
        if (apk_mount_bundled_repo() != SUCCESS) {
            newtFormDestroy(*f);
            newtPopWindow();
            errorbox("Failed to mount bundled package repository");
            return FAILURE;
        }
    }
    
    /* Step 3: Configure APK repositories */
    int network_available = SUCCESS;
    if (medium_sources == network) {
        /* For network install, check if we can reach Alpine repos */
        fprintf(flog, "Checking network connectivity for Alpine packages...\n");
        network_available = apk_check_network();
        if (network_available != SUCCESS) {
            fprintf(flog, "WARNING: Network not available, will use minimal install\n");
            /* Continue anyway - we have bundled packages */
        }
    } else {
        /* For CDROM install, try network but don't fail if unavailable */
        network_available = apk_check_network();
    }
    
    fprintf(flog, "Configuring APK repositories (network=%s)...\n", 
            network_available == SUCCESS ? "yes" : "no");
    if (apk_configure_repos("/harddisk", network_available) != SUCCESS) {
        newtFormDestroy(*f);
        newtPopWindow();
        errorbox("Failed to configure package repositories");
        return FAILURE;
    }
    
    /* Step 4: Update package indexes */
    fprintf(flog, "Updating APK package indexes...\n");
    if (apk_update_indexes("/harddisk") != SUCCESS) {
        newtFormDestroy(*f);
        newtPopWindow();
        errorbox("Failed to update package indexes");
        return FAILURE;
    }
    
    /* Step 5: Install Alpine system packages */
    fprintf(flog, "Installing Alpine system packages...\n");
    /* TODO: Add progress callback to update status window */
    if (network_available == SUCCESS) {
        /* Install from Alpine repos + bundled packages */
        if (apk_install_from_list("/harddisk", "/cdrom/ipcop/packages.txt", NULL) != SUCCESS) {
            fprintf(flog, "WARNING: Some Alpine packages failed to install\n");
            /* Continue anyway - IPCop packages are more critical */
        }
    } else {
        fprintf(flog, "Skipping Alpine packages (network unavailable)\n");
        /* Just install minimal base */
        apk_install_package("/harddisk", "alpine-base busybox");
    }
    
    /* Step 6: Install bundled IPCop packages */
    fprintf(flog, "Installing IPCop custom packages...\n");
    retcode = apk_install_ipcop_packages("/harddisk");
    
    newtFormDestroy(*f);
    newtPopWindow();

    /* abort if package installation failed */
    if (retcode != SUCCESS) {
        errorbox("Failed to install IPCop packages");
        return FAILURE;
    }
    
    fprintf(flog, "=== Package installation complete ===\n");

    /* Configure OpenRC services for first boot */
    fprintf(flog, "Configuring OpenRC services...\n");
    statuswindow(72, 5, "Post-installation", "Configuring services for first boot...");
    
    if (openrc_configure_services("/harddisk") != SUCCESS) {
        fprintf(flog, "WARNING: OpenRC service configuration had errors\n");
        /* Continue anyway - services can be configured manually */
    }
    
    /* Clean up SysVinit configuration */
    openrc_cleanup_sysvinit("/harddisk");
...
static int make_bootable(char *dev, char *dev2, int part_options)
{
    ...
    if (newworld) {
        ...
        snprintf(command, STRING_SIZE, "chroot /harddisk /usr/bin/install-quik.sh %s3 %s4", device, device);
        if (mysystem(command)) {
             return FAILURE;
        }
    }
