...
static int source_network(void)
{
    ...
    while (1) {
        ...
        snprintf(message, STRING_SIZE_LARGE, 
                 "Enter URL for IPCop APK repository\\n(e.g., http://server/ipcop/packages)");
        ...
        fprintf(flog, "Repository URL is %s\n", message);
        ...
        /* Verify if APK repository index exists */
        strcpy(filename, "APKINDEX.tar.gz");
        snprintf(command, STRING_SIZE, "wget --spider -O /tmp/%s %s/%s", filename, network_source, filename);
        rc = mysystem(command);
        newtPopWindow();
        if (!rc) {
            fprintf(flog, "NETWORK INSTALL: APK repository verified\\n");
            return SUCCESS;
        }

        /* Spider failed, inform user */
        snprintf(message, STRING_SIZE, "APK repository index not found at:\\n%s/%s", network_source, filename);
        errorbox(message);
    }
}
        ...
        if ((hardwares[i].type == cdrom) || 
            ...
            */
            ...
            for (j = 0; j < 3; j++) {
                ...
                /* Let us see if this is an IPCop Alpine installer */
                /* Check for APK repository index instead of tarball */
                strcpy(filename, "alpine/packages/APKINDEX.tar.gz");
                snprintf(filepath, STRING_SIZE, "/cdrom/%s", filename);
                if (!access(filepath, 0)) {
                    /* Valid IPCop Alpine installer found */
                    fprintf(flog, "IPCop Alpine installer detected on device %s\n", test_device);
                    newtPopWindow();
