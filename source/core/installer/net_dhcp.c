...
int changedhcpserver(void) {
  ...
  newtCenteredWindow(72, 13 + numLines,
                     gettext("TR_DHCP_SERVER_CONFIGURATION"));
  netdhcpform = newtForm(NULL, NULL, 0);
  newtFormAddComponent(netdhcpform, text);

  enabledinitvalue =
      (test_kv(dhcpkv, "ENABLED_GREEN_1", "on") == SUCCESS) ? '*' : ' ';
  enabledcheckbox = newtCheckbox(2, 2 + numLines, gettext("TR_ENABLED"),
                                 enabledinitvalue, " *", &enabledresult);
  ...
  entrystart =
      newtEntry(35, 4 + numLines, find_kv(dhcpkv, "START_ADDR_GREEN_1"), 20,
                &resultstart, 0);
  ...
  entryend = newtEntry(35, 5 + numLines, find_kv(dhcpkv, "END_ADDR_GREEN_1"),
                       20, &resultend, 0);
  ...
  entrylease =
      newtEntry(35, 6 + numLines, find_kv(dhcpkv, "DEFAULT_LEASE_TIME_GREEN_1"),
                20, &resultlease, 0);
  ...
  /* In case of changes, rewrite dhcpd.conf (basics only!) and restart DHCP
   * server */
  if (changed == TRUE) {
    if (test_kv(dhcpkv, "ENABLED_GREEN_1", "on") == SUCCESS) {
      ...
      fprintf(f, "# Do not modify '/var/ipcop/dhcp/dnsmasq.conf' directly "
                 "since any changes\n");
      fprintf(f, "# you make will be overwritten whenever you resave dhcp "
                 "settings using the\n");
      fprintf(f, "# web interface! \n");
      fprintf(f, "# Instead modify the file '/var/ipcop/dhcp/dnsmasq.local' "
                 "and then restart \n");
      fprintf(f, "# the DHCP server using the web interface or restartdhcp.\n");
      fprintf(f, "# Changes made to the 'local' file will then propagate to "
                 "the DHCP server.\n");
      fprintf(f, "\n");
      ...
      fprintf(f, "dhcp-leasefile=/var/ipcop/dhcp/dnsmasq.leases\n");
